...
server_environment = 'prod' #'test' for both dev and test
...
def addERISpoint(pointInfo,mxd,output_folder,out_points=r'points.shp'):
    ...
    if erisIDs_4points != {}:
        ...
        if check_field==[]:
            arcpy.AddField_management(out_pointsSHP, "eleRank", "SHORT", "12", "6", "", "ELEVATION_RANK", "NULLABLE", "NON_REQUIRED", "")
...
def addERISpoly(polyInfo,mxd,output_folder,out_polys=r'polys.shp'):
    ...
    if len(erisPolyIDs.keys()) > 0:
        ...
        arcpy.AddField_management(out_polysSHP, "eleRank", "SHORT", "12", "6", "", "ELEVATION_RANK", "NULLABLE", "NON_REQUIRED", "")
        arcpy.AddField_management(out_polysSHP, "multiPoly", "TEXT", "", "", "20", "MULTI_POLYGON", "NULLABLE", "NON_REQUIRED", "")
        arcpy.AddField_management(out_polysSHP, "polyCount", "SHORT", "12", "0", "", "POLYGON_COUNT", "NULLABLE", "NON_REQUIRED", "")
        ...
        del cursor
        arcpy.CalculateField_management (out_polysSHP, 'SHAPE_AREA', "!SHAPE.AREA@SQUAREFEET!", "PYTHON_9.3")
        arcpy.CalculateField_management (out_polysSHP, 'SHAPE_LEN', "!SHAPE.LENGTH@FEET!", "PYTHON_9.3")
        ...
        #erisPolysmxdLayer = arcpy.mapping.ListLayers(mxd.mxd,"Eris Polygons",df)[0]
        erisPolysmxdLayer = arcpy.mapping.ListLayers(mxd.mxd,"ERIS Polygons",df)[0]
        return erisPolysLayer,erisPolysmxdLayer
    else:
        return None, None
    ...
    if(int((arcpy.GetCount_management(masterLayer_topo).getOutput(0))) ==0):
        ...
        for cellid in cellids_selected:
            try:
                ...
                newmastertopo = r'\\cabcvan1gis007\gptools\Topo_USA\masterfile\Cell_PolygonAll.shp'                
...
if __name__ == '__main__': 
    try:
        ...
        scratch = r'C:\Users\JLoucks\Documents\JL\psr4'#arcpy.env.scratchFolder
        env = 'prod'
        ...
        erisPolysLayer, erisPolysmxdLayer=addERISpoly(erisPointsInfo,map1,scratch)
        ...
        if xplorerflag == 'Y':
            ...
            if erisPolysLayer != None:
                export_to_kml(order_num,erisPolysmxdLayer)
