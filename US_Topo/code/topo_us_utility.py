...
class topo_us_rpt(object):
    ...
    def mapDocument(self, is_nova,is_terracon,is_partner, projection):
        ...
        if cfg.yesBoundary == 'no':
            orderGeomLayer.visible = False
        elif cfg.yesBoundary == 'yes' and self.order_obj.geometry.type.lower() == 'point':
            orderGeomLayer.visible = False
        else:
            orderGeomLayer.visible = True
    ...
    def setBoundary(self, mxd, df, yesboundary):        
        ...
        print (self.order_obj.geometry.type.lower())
        arcpy.AddMessage("yesboundary = " + yesboundary)

        if yesboundary.lower() == 'fixed' and self.order_obj.geometry.type.lower() != "point" and self.order_obj.geometry.type.lower() != "multipoint":
            for lyr in arcpy.mapping.ListLayers(mxd, "", df):
    ...
    def selectTopo(self, orderGeometry, extent, projection):                
        ...
        arcpy.MakeFeatureLayer_management(cfg.mastergdb, 'masterLayer')

        arcpy.SelectLayerByLocation_management("masterLayer",'INTERSECT', orderGeometry,'0.25 KILOMETERS','NEW_SELECTION')     # select main topo images that intersect geometry
        rowsMain = [str(int(r.getValue("CELL_ID"))) for r in arcpy.SearchCursor("masterLayer")]
        arcpy.AddMessage("...selected " + str(len(rowsMain)))

        arcpy.SelectLayerByLocation_management("masterLayer",'INTERSECT', orderGeometry,'2 KILOMETERS','NEW_SELECTION')            # select adjacent topo images to main maps/intersect dataframe extent
        rowsAdj = [str(int(r.getValue("CELL_ID"))) for r in arcpy.SearchCursor("masterLayer")]
    ...
    def getTopoRecords(self, rowsMain, rowsAdj, csvfile_h, csvfile_c):
        ...
        with open(csvfile_c, "rb") as f:
            ...
            next(f)         # skip header
            for row in reader:
                if row['Cell ID'] in rowsMain or row['Cell ID'] in rowsAdj:                               # grab surrounding topo records
                    pdfname = row["Filename"].strip()
                    # if not os.path.exists(os.path.join(cfg.tifdir_c, pdfname.replace(".pdf", "_t.tif"))):
                    #     continue
    ...
    def mapSetElements(self, mxd, year, yearalldict, seriesText, quaddict):
        ...
        if self.is_terracon == 'Y':
            projNoTextE = arcpy.mapping.ListLayoutElements(mxd, "TEXT_ELEMENT", "quadText")[0]
            projNoTextE.text = 'QUADRANGLES INCLUDE: ' + quadText + ' (' + seriesText +' Min Series, '+ str(year) + ').'
        else:
        ...
        for d in dictlist:
            ...
            for year in tempyears:
                ...
                if year != "":
                    ...
                    summarydata[year+'_'+seriesText] = {'year': year, 'series':seriesText, 'source':topoSource,'quad':quadtext,'state':state,'aerial_photo_year':aerial_photo_year,'aerial_revision_year':aerial_revision_year}
            tempyears = None
